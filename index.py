...
def Database():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS admin (admin_id INT AUTO_INCREMENT PRIMARY KEY, username VARCHAR(255), password VARCHAR(255))")
    cursor.execute("CREATE TABLE IF NOT EXISTS product (product_id INT AUTO_INCREMENT PRIMARY KEY, product_name VARCHAR(255), product_qty INT, product_price INT)")
...
def addNew():
    ...
    if name and qty and price:
        try:
            cursor.execute("INSERT INTO products (name, qty, price) VALUES (%s, %s, %s)", (name, qty, price))
            conn.commit()
...
def Reset():
    tree.delete(*tree.get_children())
    DisplayData()  # Assuming DisplayData() exists and handles database operations
    SEARCH.set("")
    ...
    if not tree.selection():
        ...
        if result == 'yes':
            ...
            try:
                ...
                cursor.execute(query, (selecteditem[0],))
                conn.commit()
